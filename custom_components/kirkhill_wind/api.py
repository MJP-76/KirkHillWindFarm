...
class KirkHillWindApi:
    ...
    def __init__(self, base_url: str, api_key: str):
        ...
        # Sanitizes the base url layout safely
    ...
    async def _request(self, session: aiohttp.ClientSession, endpoint: str) -> dict:
        ...
        try:
            _LOGGER.debug(f"Calling clean Kirkhill path: {url}")
            async with session.get(url, headers=headers, timeout=15) as response:
                response.raise_for_status()
                return await response.json()
        except Exception as err:
            _LOGGER.error(f"Kirkhill request failed at {url}: {err}")
            raise
